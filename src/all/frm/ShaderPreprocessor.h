...
public:
	...
	/// Open a file, process and append to the source result. 
	/// \return false if an error occurred.
	bool process(const char* _fileName);

	/// Directly append _str to the source result with an optional
	/// newline char.
	void append(const char* _str, bool _newLine = false);

	/// Get the source result. Subsequent calls to append() or process() will
	/// invalidate the ptr returned by this method. Note that this may not
	/// be a valid null-terminated string if process() failed.
	const char* getResult() const { return (const char*)m_result.data(); }

private:
	std::vector<char>             m_result;    //< Source code result.
	std::vector<apt::String<64> > m_included;  //< Prevent recursive includes.
	
	/// Process an include directive. _tp points to the first char after an
	/// #include directive. Extract filename, check that the file wasn't already
	/// included and call process().
